...
export const Route = createFileRoute("/track")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track your Shri Radha Govind Store order in real time using your Order Number or Tracking ID. See courier, status and delivery updates." },
  ...
  const lookup = async (term: string) => {
    ...
    try {
      if (!isApiEnabled()) {
        ...
        const found = orders.find((o: any) => (o.trackingId ?? "").toUpperCase() === t || String(o.orderNo ?? "") === t || o.id === t);
  ...
  return (
    <Layout>
      <div className="container-app py-10 max-w-3xl">
        ...
        <p className="text-sm text-muted-foreground mt-1">Enter your Order Number (e.g. #5001) or Courier Tracking ID (AWB) to track your shipment live.</p>
        <form onSubmit={(e) => { e.preventDefault(); lookup(id); }} className="mt-6 flex gap-2">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <input value={id} onChange={(e) => setId(e.target.value)} placeholder="e.g. 5001 or U2000337996" className="w-full h-12 pl-10 pr-4 rounded-full border bg-card focus:outline-none focus:border-primary text-sm uppercase tracking-wider" />
          </div>
          ...
          <div className="mt-8 space-y-6">
            <div className="premium-card p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  {order.orderNo && (
                    <p className="font-display text-2xl font-bold text-primary">Order #{order.orderNo}</p>
                  )}
                  {order.trackingId ? (
                    <div className="mt-1 inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-teal-50 border border-teal-200 text-teal-900 text-xs font-mono">
                      <span className="font-sans text-[10px] uppercase font-bold text-teal-700">AWB:</span>
                      <span className="font-bold">{order.trackingId}</span>
                    </div>
                  ) : (
                    <p className="text-xs text-muted-foreground mt-1">Tracking ID: <i>Will be assigned once shipped</i></p>
                  )}
                  <p className="text-xs text-muted-foreground mt-1.5">Placed on {new Date(order.createdAt).toLocaleString()}</p>
                </div>
