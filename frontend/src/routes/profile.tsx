...
import { displayOrderNumber, formatINR, useStore, type Address } from "@/lib/store";
      ...
      <div className="container-app py-10 lg:py-14">
        ...
        <div className="mt-7 grid gap-6 lg:grid-cols-[240px_1fr]">
          <aside className="premium-card h-fit space-y-1 p-3 lg:sticky lg:top-40">
            ...
            <Link to="/wishlist" className="flex w-full items-center gap-3 rounded-lg px-3 py-3 text-sm font-medium text-muted-foreground transition hover:bg-muted hover:text-primary"><Heart className="h-4 w-4" /> Wishlist <span className="ml-auto rounded-full bg-primary/10 px-2 py-0.5 text-xs text-primary">{wishlist.length}</span></Link>
          </aside>
                ...
                <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
                  {section === "details" ? (
                    <>
                      ...
                      <div className="rounded-lg border bg-secondary/40 p-4 text-sm"><p className="flex items-center gap-2 font-semibold"><CheckCircle2 className="h-4 w-4 text-primary" /> Account verified</p><p className="mt-1 text-xs text-muted-foreground">Your email is permanently linked to this account and cannot be changed.</p></div>
                    </>
                  ) : (
                    <>
...
function SummaryButton({ icon: Icon, title, text, onClick }: { icon: typeof UserRound; title: string; text: string; onClick: () => void }) { return <button onClick={onClick} className="group flex items-center gap-3 rounded-lg border bg-card/60 p-4 text-left transition hover:-translate-y-0.5 hover:border-primary"><span className="grid h-10 w-10 place-items-center rounded-lg bg-primary/10 text-primary"><Icon className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-semibold">{title}</span><span className="block truncate text-xs text-muted-foreground">{text}</span></span><ChevronRight className="h-4 w-4 transition group-hover:translate-x-1" /></button>; }
function ProfileNav({ active, onClick, icon: Icon, children }: { active: boolean; onClick: () => void; icon: typeof UserRound; children: React.ReactNode }) { return <button onClick={onClick} className={`flex w-full items-center gap-3 rounded-lg px-3 py-3 text-sm font-medium transition ${active ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:bg-muted hover:text-primary"}`}><Icon className="h-4 w-4" />{children}</button>; }
...
function ProfileField({ label, value, onChange, disabled, locked = false, full = false, icon: Icon }: { label: string; value: string; onChange: (value: string) => void; disabled: boolean; locked?: boolean; full?: boolean; icon: typeof UserRound }) {
  return <label className={full ? "sm:col-span-2" : ""}><span className="text-xs font-medium text-muted-foreground">{label}</span><span className="relative mt-1 block"><Icon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={value} onChange={(event) => onChange(event.target.value)} disabled={disabled} className={`h-12 w-full rounded-lg border pl-10 pr-10 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10 ${disabled ? "bg-muted/50 text-muted-foreground" : "bg-card"}`} />{locked && <LockKeyhole className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />}</span></label>;
}

function OrderHistory({ orders }: { orders: ReturnType<typeof useStore>["orders"] }) {
  return <section className="premium-card overflow-hidden"><div className="border-b p-5 sm:p-6"><h2 className="font-display text-2xl">Order history</h2><p className="text-sm text-muted-foreground">Track every purchase and delivery in one place.</p></div>{orders.length === 0 ? <div className="p-12 text-center"><ShoppingBag className="mx-auto h-10 w-10 text-primary/50" /><h3 className="mt-3 font-display text-xl">No orders yet</h3><p className="mt-1 text-sm text-muted-foreground">Your devotional purchases will appear here.</p><Link to="/shop" className="fx-button mt-5 inline-flex h-10 items-center rounded-full bg-primary px-5 text-sm text-primary-foreground">Start shopping</Link></div> : <div className="divide-y">{orders.map((order) => <Link key={order.id} to="/orders/$id" params={{ id: order.id }} className="group flex flex-wrap items-center gap-4 p-5 transition hover:bg-secondary/40"><span className="grid h-12 w-12 place-items-center rounded-lg bg-primary/10 text-primary"><Package className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block font-semibold">Order #{displayOrderNumber(order)}</span><span className="block text-xs text-muted-foreground">{new Date(order.createdAt).toLocaleDateString("en-IN", { dateStyle: "medium" })}  -  {order.items.length} item{order.items.length === 1 ? "" : "s"}</span></span><span className="text-right"><span className="block font-semibold">{formatINR(order.total)}</span><span className="rounded-full bg-primary/10 px-2 py-1 text-xs text-primary">{order.status}</span></span><ChevronRight className="h-4 w-4 transition group-hover:translate-x-1" /></Link>)}</div>}</section>;
}
