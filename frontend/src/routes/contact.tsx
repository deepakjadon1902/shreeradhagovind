    ...
    <Layout>
      <section className="container-app py-12 md:py-16">
        ...
        <div className="grid lg:grid-cols-5 gap-6 mt-10">
          ...
          <form onSubmit={submit} className="lg:col-span-3 bg-card border border-border rounded-lg p-6 md:p-8 premium-shadow space-y-4">
            ...
            <label className="block text-sm">
              ...
              <textarea required rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="mt-1 w-full rounded-lg border border-border bg-background p-3 focus:outline-none focus:border-primary" />
            </label>
...
function Input({ label, value, onChange, type = "text", required }: { label: string; value: string; onChange: (v: string) => void; type?: string; required?: boolean }) {
  return (
    <label className="block text-sm">
      ...
      <input required={required} type={type} value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full h-11 rounded-lg border border-border bg-background px-3 focus:outline-none focus:border-primary" />
    </label>
  );
}
