...
function BlogPost() {
  ...
  const post = blogs.find((b) => (b.slug === slug || slugify(b.title) === slug) && b.isPublished);
  if (!post) {
    return (
      <Layout>
        <div className="container-app py-20 text-center">
          ...
          <Link
            to="/blog"
            className="mt-5 inline-flex h-10 items-center rounded-full bg-primary px-5 text-sm text-primary-foreground"
          >
            Back to blog
          </Link>
        </div>
