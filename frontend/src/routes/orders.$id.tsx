    ...
    return (
      <Layout>
        <div className="container-app py-20 text-center">
          ...
          <Link to="/orders" className="text-primary mt-4 inline-block">
            All orders
          </Link>
        </div>
      </Layout>
  ...
  return (
    <Layout>
      <div className="container-app py-10">
        ...
        <h1 className="font-display text-4xl mt-2">Order #{displayOrderNumber(order)}</h1>
          ...
          <div className="premium-card p-6 mt-6">
            ...
            <div className="flex items-center justify-between relative">
              ...
              <div
                ...
                style={{ width: `calc(${(idx / (STAGES.length - 1)) * 100}% - 0px)` }}
              />
              {STAGES.map((s, i) => (
                <div key={s} className="relative z-10 flex flex-col items-center text-center">
                  <div
                    className={`h-10 w-10 rounded-full grid place-items-center ${i <= idx ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                  >
                    {i <= idx ? (
                  ...
                  <p
                    className={`text-[10px] mt-2 max-w-[80px] ${i <= idx ? "text-foreground font-medium" : "text-muted-foreground"}`}
                  >
                    {s}
                  </p>
                </div>
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-6 mt-6">
          ...
          <aside className="space-y-4">
            ...
            <div className="premium-card p-5">
              ...
              <p
                className={`text-xs mt-1 ${order.payment.status === "paid" ? "text-green-700" : "text-amber-700"}`}
              >
