...
export const Route = createFileRoute("/cart")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Review the sacred essentials in your bag and proceed to a secure checkout.",
      },
      ...
      <Layout>
        <div className="container-app py-20 text-center">
          ...
          <p className="text-muted-foreground mt-2">
            Add sacred treasures from Vrindavan to your cart.
          </p>
          <Link
            to="/shop"
            className="mt-6 inline-flex h-12 items-center rounded-md bg-primary px-8 font-semibold text-primary-foreground"
          >
            Continue shopping
          </Link>
        </div>
      ...
      <div className="container-app py-6 md:py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
          <div className="space-y-3">
            {shipping === 0 ? (
              <div className="flex items-center gap-2 rounded-lg bg-emerald-600/10 px-4 py-3 text-sm text-emerald-700">
                <Truck className="h-4 w-4" /> Yay! You get <strong>FREE delivery</strong> on this
                order.
              </div>
            ) : (
              ...
              return (
                ...
                >
                  <Link
                    ...
                    className="h-28 w-28 shrink-0 overflow-hidden rounded-lg bg-white sm:h-32 sm:w-32"
                  >
                    <img
                      src={i.product.image}
                      alt={i.product.name}
                  ...
                  <div className="flex-1 min-w-0 flex flex-col">
                      ...
                      {off > 0 && (
                        <>
                          <span className="text-xs text-muted-foreground line-through">
                            {formatINR(i.product.mrp)}
                          </span>
                          <span className="text-xs font-semibold text-emerald-700">{off}% off</span>
                        </>
                      )}
                    </div>
                    ...
                    <div className="mt-auto pt-3 flex items-center gap-2 flex-wrap">
                      ...
                      >
                        <Heart
                          className={`h-3.5 w-3.5 ${wished ? "fill-destructive text-destructive" : ""}`}
                        />{" "}
                        Save for later
                      </button>
                      <button
            ...
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <div className="space-y-2.5 text-sm">
                ...
                <Row
                  ...
                  valueClass={shipping === 0 ? "text-emerald-700 font-medium" : ""}
                />
...
function Row({
  ...
  return (
    <div
      className={`flex justify-between ${bold ? "font-bold text-base pt-1" : "text-foreground/80"}`}
    >
      <span>{label}</span>
